...
import { getListData } from "../../services/NeoWPService";
...
class ObjectList extends Component {
    ...
    async componentDidMount() {
        const listData = this.props.match.params.listData;

        try {
            const data = await getListData(listData);
        ...
        return (
            <div className="container">
                ...
                <div className="row border-bottom mt-1 mb-4">
                    <div className="col">
                        <h1>{listData} : Prévisions sur 7 jours</h1>
                    </div>
                </div>
                {loader}
                {error}
                {this.state.data &&
                Object.keys(this.state.data).map((dateObject, i) => (
                    <div key={i}>
                        <h2 className="mt-4 border-bottom">{dateObject}</h2>
                        <div className="d-flex flex-column flex-lg-row justify-content-center align-items-center">
                            {_.get(this.state.data, dateObject).map(el => (
                                <ObjectListItem
                                    timestamp={el.dt}
                                    key={el.dt}
